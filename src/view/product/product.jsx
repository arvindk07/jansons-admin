...
const products = [
  {
    ...
    size: "40x40x90 cm",
    image:
      "https://developers.elementor.com/docs/assets/img/elementor-placeholder-image.png", // Replace with a real image URL if available
  },
  {
    id: 2,
...
const Product = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (isUpdate) {
      await axios
        .put(`https://api.jainsonsindiaonline.com/api/product/edit`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      await axios
        .post(`https://api.jainsonsindiaonline.com/api/product/add`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "multipart/form-data",
          },
  ...
  const handleAddCategory = async (newCategory) => {
    try {
      const response = await axios.post(
        "https://api.jainsonsindiaonline.com/api/categories/add",
        { name: newCategory },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        "https://api.jainsonsindiaonline.com/api/product/delete",
        {
          data: {
            id: id,
          },
  ...
  const exportToExcel = () => {
    ...
    function findFeature(header, features) {
      const feat = features.find(
        (ftr) => header.toLowerCase() == ftr["key"].toLowerCase()
      );
      return feat?.value;
    }
    ...
    <div style={{ height: "100%" }}>
      ...
      >
        ...
        <Box display={"flex"} gap={2} alignItems={"center"}>
          <IconButton
            ...
            color={viewType === "Table" ? "secondary" : ""}
          >
            <TableChartIcon />
          </IconButton>
          <IconButton
            onClick={() => setViewType("Grid")}
            color={viewType === "Grid" ? "secondary" : ""}
          >
            <AppsIcon />
          </IconButton>
          <div className="">
      ...
      >
        <form
          ...
          style={{ padding: "40px 20px", width: "600px", marginTop: "40px" }}
        >
          <Box
            sx={{
              display: "flex",
          ...
          <Box mb={2}>
            ...
            {allImages?.map((data, index) => (
              ...
              >
                ...
                />
                <IconButton onClick={() => DeleteImages(index)}>
                  <DeleteIcon />
                </IconButton>
              </Box>
            ))}
                ...
                render={({ field, fieldState }) => (
                  <>
                    {features[index].key === "Stock Movement" ? (
                      <>
                        ...
                        <Box>
                          <FormControlLabel
                            control={
                              <Checkbox
                                value={"Fast Moving Stock"}
                                onChange={() =>
                                  updateFeature(index, "Fast Moving Stock")
                                }
                                checked={
                                  features[index].value === "Fast Moving Stock"
                                }
                              />
                            }
                            label="Fast Moving Stock (A)"
                          />
                          <FormControlLabel
                            control={
                              <Checkbox
                                ...
                                onChange={() =>
                                  updateFeature(index, "Slow Moving Stock")
                                }
                                checked={
                                  features[index].value === "Slow Moving Stock"
                                }
                              />
                            }
                            label="Slow Moving Stock (A)"
                          />
                        </Box>
                        <p>{fieldState.error?.message}</p>
                      </>
                      ...
                      <>
                        {console.log(
                          features[index].key === "Description",
                          "Hello"
                        )}
                        <InputLabel>{features[index].key}</InputLabel>
                        <TextField
                      ...
                      {productData
                        ?.filter((product) => {
                          ...
                          const brandFeature = product.features.find(
                            (feature) => feature.key.toLowerCase() === "brand"
                          );
                          const brandName = brandFeature
                            ? brandFeature.value.toLowerCase()
                            : "";

                          return (
                            (selectdropDownValue
                              ? product.category === selectdropDownValue
                              : true) &&
                            (searchText
                              ? product.name
                                  .toLowerCase()
                                  .includes(searchText.toLowerCase()) ||
                                brandName.includes(searchText.toLowerCase()) // 🔍 Search by brand
                              : true)
                          );
                        })
                        ?.slice(
                        ...
                        .map((row, index) => (
                          <TableRow key={row.id}>
                            ...
                            {row?.features?.map((data) => (
                              <TableCell key={data._id}>{data.value}</TableCell>
                            ))}
                          </TableRow>
                        ))}
                    </TableBody>
                    ...
                    <Grid item xs={12} sm={6} md={4} key={index}>
                      <Card>
                        ...
                        <CardContent>
                          ...
                          {product?.features?.map((data) => (
                            <Typography variant="body2" key={data._id}>
                              {data?.key}: {data?.value}
                            </Typography>
                          ))}
                          <Box sx={{ mt: 2 }}>
