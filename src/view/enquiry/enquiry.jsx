...
const Enquiry = () => {
  ...
  const getCategory = (catId) => {
    if (categoryData) {
      const catF = categoryData?.find((cat) => cat._id == catId);
      return catF.name;
    }
    return null;
  };
    ...
    const getData = async (url) => {
      ...
      try {
        const response = await axios.get(
          `https://api.jainsonsindiaonline.com/api/contactUs/showAll${paramsFilter}`,
          {
            headers: {
              Authorization: `Bearer ${getToken}`, // Add the token to the Authorization header
            },
            ...
            <TableHead>
              <TableRow>
                {Object.keys(contactUsData[0] || {})?.map((key, index) => {
                  return !forgetThese.includes(key) && key !== "updatedAt" ? ( // Ensuring updatedAt is excluded
                    <TableCell key={index}>
                      {key === "createdAt" ? "CREATED AT" : key.toUpperCase()}
                    </TableCell>
                  ) : null;
                })}
              </TableRow>
              ...
              {contactUsData?.map((item, index) => (
                <TableRow key={index}>
                  {Object.entries(item || {}).map(([key, value], idx) =>
                    !forgetThese.includes(key) && key !== "updatedAt" ? (
                      <TableCell key={idx}>
                        {typeof value === "object"
                          ? transformData(value)
                          : key !== "category"
